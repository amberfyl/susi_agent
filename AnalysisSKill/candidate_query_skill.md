...
## 4. Chip key 說明（資料面）
- `config_new.db` 的 `ProductChip` 可能存在多種配對：
  - `platform + EC chip`
  - `platform + PCH chip`
...
2. 取命中 ProductChip 的 `hardware_id` 後，查各 section table

### 5.1 I2C
- 只有 `{project}-spec.json` 的 `features.i2c == true` 才進行 I2C query/generate；false 或缺值時維持 `SECTION_EMPTY`。
- query 只查 `config_new.db` 中實際存在的 `I2C` row；沒有對應 row 的組合不做 fallback，維持 `SECTION_EMPTY`。
- 目前 `I2C` table 以 `hardware_id` 對應 ProductChip；候選組合由正式 DB 的實際 rows 決定，不再使用 `prod_chip_id` join。
...
- `FOUND`：section 有候選
- `INVALID_SECTION`：輸入 section 不在目前 `config_new.db` table 列表

## 8. Deterministic CLI（程式化查詢）
- 腳本：`/home/company2/AIagent_susi/query_config_db.py`
- 用法：
  - `python3 query_config_db.py <product_name> <chip_name> <section> [--db /path/to/config_new.db]`
...
## 10. HWM.Voltage 資料契約（僅資料層）

### 10.1 目的
- 提供含 `report_name`、`channel_name`、`channel_id` 的 hardware-specific rows，供 orchestrator 在 EC 流程中做 mapping 使用。

### 10.2 契約
- `report_name`：對齊 probe report 的 `HWM_VOLTAGE_*` 列舉名稱。
- `channel_id`：對應 INI `HWM.Voltage` 中 `Channel` 欄位值。
- 本 section table 可不覆蓋 probe 全列舉；未收錄者視為例外，不代表錯誤。
...
- alias 不在 query 層裁決；交由 orchestrator 依 BIOS + probe + section row 融合回填

### 11.2 非EC（`is_ec=False` 或未知）
1. 仍 query `HWM.Voltage` table（同固定 key，不跳過）
2. 以 query 候選作為 base
3. SuperIO（NCT6694B / NCT61xxD / NCT6776D）採 v1 種子模式：
   - 不先要求 report name 對位
   - 先把查到的 channel rows 全部輸出（供 target 機驗證）
4. v2 再由 target 成功回報通道 + BIOS 名稱/電壓值回推 item 對位
   - 僅保留 BIOS 可見電壓 item
   - Name(alias) 由 BIOS 顯示名稱回填
5. 若仍有 channel duplicate 且缺證據，再交由 R-014 / R-015 或標記 pending

### 11.3 邊界
- query skill 只提供 DB 候選與 mapping 資料，不做 R-014/R-015 判圖執行。
- R-014/R-015 的判圖修正屬 orchestrator / diagram filter skill 職責。

### 11.4 非EC SuperIO 的 HWM.Temperature（對齊 voltage 同邏輯）
1. 先 query `HWM.Temperature` table（固定 key：`(product_name, chip_name)`）
2. 對 NCT6694B / NCT61xxD / NCT6776D：v1 直接輸出全部 channel rows（不先要求 report name 對位）
3. v2 由 target probe [OK] 通道 + BIOS 溫度名稱/數值做交集收斂
   - 只保留 BIOS 可見且 probe 成功的 item
   - Name(alias) 由 BIOS 顯示名稱回填
4. query skill 本身不做跨來源融合與最終 item 裁決（由 orchestrator 執行）
